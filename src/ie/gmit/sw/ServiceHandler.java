...
// Declaration of Service Handler class
public class ServiceHandler extends HttpServlet {
	...
	// private Deque<Request> INQUEUE = new LinkedList()
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//To force download!
		//resp.setContentType("file that browser cannot handle/html");
		...
		// These are coming back as strings, be careful when you want an integer
		...
		// This executes first time only
		if (taskNumber == null){
			taskNumber = new String("T" + jobNumber);
			jobNumber++;
			//Add job to in-queue
		}else{
			// This can execute the next time
		...
		// Break sections into individual shingles
		r = new Reader(section, taskNumber);
		// Read, Create & return new shingles
		shingleList = r.newShingle();
		
		// Creation of new document that will be passed to the service thread
		d = new Document(taskNumber, title, shingleList);
		
		// Making new Service Job to serve a new document
		sj = new ServiceJob(d);
